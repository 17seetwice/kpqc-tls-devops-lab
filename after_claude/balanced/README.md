...
프로그램은 디스크에 저장된 실행 파일이고, 프로세스는 그 프로그램이 실제로 실행 중인 상태다. 예를 들어 `tls_handshake` 파일을 실행하면 운영체제가 메모리와 실행 상태를 마련하고 프로세스 번호(PID, Process ID)를 부여한다. 같은 프로그램을 별도로 두 번 실행하면 서로 다른 프로세스가 생길 수 있다.

이번 실험의 프로세스는 서버·클라이언트 시험 프로그램을 뜻한다. EC2 인스턴스나 컨테이너를 매번 새로 만드는 것이 아니며, 포트가 열려 있는지만으로 프로세스 재사용 여부를 구분하지 않는다.

### 새 프로세스와 프로세스 재사용의 차이

- 새 프로세스(cold): 클라이언트 프로그램을 연결마다 새로 실행한다. 서버는 대기 중인 부모 프로세스가 연결마다 자식 프로세스를 만들고, 자식이 TLS 설정을 준비해 핸드셰이크를 처리한다.
- 프로세스 재사용(warm): 한 암호 구성의 반복 측정 동안 같은 프로세스와 TLS 설정을 유지한다. TLS 설정을 담는 OpenSSL의 `SSL_CTX`를 재사용하며, 준비 연결 2회 후 3회를 분석한다.
- 공통 조건: 연결마다 새 TCP (Transmission Control Protocol) 연결과 개별 TLS 연결 객체를 만들고 전체 핸드셰이크를 수행한다. 세션 티켓을 이용해 인증·키 교환 절차를 줄이는 TLS 세션 재개는 사용하지 않는다.

```text
새 프로세스:   새 실행·설정 준비 → 연결·전체 핸드셰이크 → 종료 → 다음 연결은 새 실행
프로세스 재사용: 실행·설정 준비 → 연결·전체 핸드셰이크 → 연결 종료 → 같은 실행에서 새 연결
```

재사용 범위는 한 구성의 반복 연결이다. 모든 암호 조합을 하나의 프로세스로 측정한 것은 아니다. cold도 운영체제 캐시를 강제로 비운 상태를 뜻하지 않는다.
