...
기존 TLS 성능 측정과 암호 정책 게이트를 연결해 **레거시 서비스에서 KPQC로의 전환 → 성능 기반 배포 승인 → 장애 시 이전 승인 KPQC 버전 복구**를 검증했다. [GitHub Actions 실행](https://github.com/17seetwice/kpqc-tls-devops-lab/actions/runs/36032662708), 소스 `c232f7d`. 24개 검증 항목 통과. 워크플로는 기존 두 EC2의 중지 완료와 임시 SSH 규칙 제거를 확인했다.
...
- 장애 주입 요청부터 첫 복구 성공 확인: **2.951초** (사전 목표10초).
- 장애 관측 부하:150회 중 실패 **21회**, 신규 배포 서비스 성공 23회, 복구된 이전 서비스 성공 106회.
