...
국산 양자내성암호를 통합한 OpenSSL에서 TLS 1.3 핸드셰이크의 지연·자원 사용을 측정하고, 실제 협상 결과를 이용해 배포 후보의 정책 적합성을 검증한 실험 기록이다. 이 폴더는 기존 원자료를 재분석·정리한 것으로 새로운 AWS 실행을 포함하지 않는다.
...
| 측정 조건 | X25519 + ECDSA | PQC 구성별 중앙값 범위 |
|---|---:|---:|
