...
p=argparse.ArgumentParser();p.add_argument('--server');p.add_argument('--client');p.add_argument('--server-private');p.add_argument('--image',default='kpqc-lab:gate');p.add_argument('--local',action='store_true');p.add_argument('--smoke',action='store_true');a=p.parse_args()
...
try:
    ...
    if a.local:
        # Connect client to the server bridge for the local integration test only.
    ...
    for name in ['mixed-kem','good-v2']:
        ...
        check('under-load-'+name+'-traffic',not worker('client',{'action':'load-progress'})['done'])
