...
assert len(C['instances'])==2 and all(x['State']=='stopped' for x in C['instances'])
...
for lang in ['ko','en']:
 ...
 names=['지연 주입 후보','정상 KPQC v2','정상 KPQC v3'] if ko else ['Injected-delay candidate','Healthy KPQC v2','Healthy KPQC v3']
 ...
 kinds=[('v3','#0072B2','장애 대상 v3' if ko else 'Faulted v3'),('failed','#D55E00','실패 접속' if ko else 'Failed attempts'),('v2','#009E73','복구된 v2' if ko else 'Restored v2')]
...
기존 TLS 성능 측정과 암호 정책 게이트를 연결해 **레거시 서비스에서 KPQC로의 전환 → 성능 기반 배포 승인 → 장애 시 이전 승인 KPQC 버전 복구**를 검증했다. [GitHub Actions 실행]({url}), 소스 `{A['source_commit'][:7]}`. {A['assertions']}개 검증 항목 통과. 기존 두 EC2는 실행 후 중지 상태를 별도로 확인했다.

## 실행 환경과 승인 정책

서울 동일 가용 영역의 m7i.large 서버·클라이언트 두 대, 컨테이너별 2 CPU/512 MiB, MTU (Maximum Transmission Unit) 1500의 Docker 브리지 환경이다. 초기 서비스는 X25519 + ECDSA P-256, KPQC 후보는 SMAUG1 + HAETAE2다. TLS (Transport Layer Security) 1.3의 전체 핸드셰이크를 사용하며, 직접 신뢰한 서버 인증서와 사전 준비된 전환 가능 클라이언트를 사용한다.

SLO (Service Level Objective)는 시험 전에 고정한 **모의 서비스 목표**다. 금융권 규정·알고리즘 안전성 기준이 아니다. 후보마다 초당10회, 10초 ×3구간의 고정 도착률로 시험한다. 각 구간에서 실패율≤1%, 200ms 내 성공 비율≥99%, 성공 접속 완료 지연 p95≤200ms를 요구한다. 부하 생성 지연 p95가50ms를 초과하거나 증적이 누락되면 승인하지 않는다.
...
모든 후보의 암호 검사는 통과했다. 지연 주입 후보에는 서버의 TLS 처리 시작 전350ms 대기를 의도적으로 넣었다. 이는 성능 회귀 검출 시험이며 PQC 알고리즘의 본래 성능이 아니다. 아래 각 값은100개 예정 시도의 한 측정 구간에서 구한 성공 접속 완료 지연 p95(ms)다.
...
지연 후보의 승격 요청은 거절되고 기존 레거시 서비스가 유지됐다. 정상 v2의 승인 후 실제 서버 인증서와 협상 코드가 KPQC로 바뀌었으며, 이어서 정상 v3도 승인됐다. 성능 측정 증적은 후보 세대·인증서·정책 식별자와 연결하고 승인 시 다시 판정한다.

## 장애 복구 결과

v3 서버 프로세스 그룹을 중단한 뒤, 상태 관측에서 연속 두 번 실패하면 이전 v2의 현재 접속·인증서·승인 이력을 확인해 자동 복구했다. 고전 버전, 다른 인증서, 오래된 복구 증적은 거절했다.

- 장애 주입 요청부터 감지: **{A['detection_ms']/1000:.3f}초**.
- 장애 주입 요청부터 첫 복구 성공 확인: **{A['recovery_ms']/1000:.3f}초** (사전 목표10초).
- 장애 관측 부하:150회 중 실패 **{A['recovery_failures']}회**, v3 성공 {A['old_pqc_successes']}회, 복구된 v2 성공 {A['restored_pqc_successes']}회.
- 마지막20개 접속은 모두 v2로 성공했고, 복구 후 암호 정책과 고전 접속 거절을 재검증했다.
...
이 실험은 제한된 부하에서 승인·거절과 복구 경로의 기능을 보여준다. 최대 처리량·금융 업무 SLO·장기간 운영 가용성·자동 코드 변환을 입증하지 않는다. 고전→KPQC 전환과 이후 KPQC v3→v2 복구를 구분하며, 전환 후 고전 서비스로 복귀하지 않는다.
...
The existing TLS measurement and cryptographic gate now form one lifecycle: **classical-to-KPQC migration, performance admission, and automatic recovery to an approved KPQC version**. [GitHub Actions execution]({url}), source `{A['source_commit'][:7]}`; {A['assertions']} assertions passed. Both existing EC2 instances were independently confirmed stopped.

## Environment and preregistered target

Two m7i.large instances in the same Seoul availability zone; each container has2 CPUs/512MiB, a Docker bridge and MTU (Maximum Transmission Unit)1500. The original service uses X25519 + ECDSA P-256; candidates use SMAUG1 + HAETAE2. Full TLS (Transport Layer Security)1.3 handshakes, directly trusted server certificates and migration-capable clients are assumed.

The SLO (Service Level Objective) is a **synthetic service target**, fixed before testing, not a financial regulation. Each candidate receives10 scheduled arrivals/s for three10-second windows. Every window must have≤1% failed attempts,≥99% successful completions within200ms, and successful completion p95≤200ms. Generator-lateness p95>50ms or incomplete evidence prevents admission.
...
All candidates passed cryptographic checks. The faulted candidate intentionally waits350ms before server TLS processing. This validates regression detection and is not intrinsic KPQC cost. Each value below is successful completion p95(ms) from one100-arrival window.
...
The slow candidate could not be promoted; the classical service remained active. Approved v2 then replaced it, verified through the actual certificate and negotiated codes. Healthy v3 was subsequently admitted. Performance evidence is bound to the candidate generation, certificate and policy and re-evaluated at promotion.

## Failure and automatic recovery

After terminating the v3 server process group, two consecutive failed health observations trigger validation and restoration of the previously approved v2. Classical targets, incorrect certificates and stale health evidence are rejected.

- Injection request to detection: **{A['detection_ms']/1000:.3f}s**.
- Injection request to first verified recovery: **{A['recovery_ms']/1000:.3f}s**, against a10-second target.
-150 scheduled fault-window attempts: **{A['recovery_failures']} failures**, {A['old_pqc_successes']} successful v3 connections, {A['restored_pqc_successes']} successful restored-v2 connections.
- The final20 attempts succeeded on v2; cryptographic policy and prohibited-client rejection were checked again.
...
Idle accept-timeout exits in the prefork server were fixed; all eight workers and a successful connection were checked after17 seconds idle. The former approved service stayed available while the next candidate was measured.

An earlier full Mac amd64-emulation trial rejected a healthy candidate because2/100 attempts exceeded200ms. That failed trial is preserved; the target was not relaxed. Native Linux preflight and AWS execution are separate. These figures describe only the AWS execution.

This bounded trial validates admission and recovery, not maximum capacity, banking-service SLOs, long-term availability or automatic source-code migration. Initial classical-to-KPQC migration and subsequent KPQC v3-to-v2 recovery are distinct. No classical rollback is allowed after migration.
