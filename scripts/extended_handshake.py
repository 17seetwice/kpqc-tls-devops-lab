...
ap=argparse.ArgumentParser();ap.add_argument('--server');ap.add_argument('--client');ap.add_argument('--server-private');ap.add_argument('--image',default='0923_kpqc_devops-gate');ap.add_argument('--local',action='store_true');ap.add_argument('--smoke',action='store_true');ap.add_argument('--balanced',action='store_true',help='ten paired latency blocks with balanced randomized mode order; no memory run');args=ap.parse_args()
...
if args.balanced:
    D['measurement']['blocks']='10 paired blocks; 5 cold-first and 5 warm-first in shuffled order; matching configuration order within each pair; one EC2 execution'
    D['measurement']['memory']='not collected in this latency-only run'
D['source_commit']=os.environ.get('GITHUB_SHA')
D['random_seed']=20260926 if args.balanced else 20260925
started=[]
    ...
    if args.balanced:
        ...
        for mode in ['cold','warm','memory']:
            for block in range(1 if args.smoke or mode=='memory' else 5):
                order=list(pairs);rng.shuffle(order);schedule.append((mode,block,order))
    D['schedule']=[{'mode':m,'block':b,'configuration_order':o} for m,b,o in schedule]
    for mode,block,order in schedule:
        ...
        rd['ended_at']=datetime.now(timezone.utc).isoformat();save()
