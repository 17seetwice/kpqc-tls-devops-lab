...
def deploy(image, extended=False, systems=False, release=False):
    ...
    try:
        ...
        if release:
            subprocess.run(['python3','scripts/release_experiments.py','--server',nodes[ids['server']]['PublicIpAddress'],
                            '--client',nodes[ids['client']]['PublicIpAddress'],'--server-private',nodes[ids['server']]['PrivateIpAddress'],
                            '--image',image],check=True,cwd=ROOT,timeout=1200)
        if systems:
...
if __name__ == '__main__':
    ...
    assert sum([args.extended,args.systems,args.release])<=1, 'Run one follow-up suite at a time'
    cleanup() if args.cleanup else deploy(args.image,args.extended,args.systems,args.release)
